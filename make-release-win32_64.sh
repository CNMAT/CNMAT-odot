archive_name=odot-Max-Win32-64-`git describe --tags --long`-`git branch | egrep '^\*' | awk '{print $2}'`.zip

deprecated_externs=(o.expr o.message o.pak o.unless o.when)
dev_externs=(o.context o.downcast o.mappatch o.messageiterate o.slip.decode o.slip.encode o.table o.validate)
legit_externs=(o.append o.atomize o.change o.collect o.compose o.cond o.dict o.difference o.display o.edge~ o.explode o.expr.codebox o.flatten o.if o.intersection o.listenumerate o.pack o.prepend o.print o.printbytes o.route o.schedule o.select o.timetag o.union o.var)
